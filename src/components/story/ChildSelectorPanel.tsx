...
export function ChildSelectorPanel({ children, selectedId, onSelect }: ChildSelectorPanelProps) {
  return (
    <div className="flex flex-col w-20 md:w-44 flex-shrink-0 gap-2">
      <p className="text-[10px] md:text-xs text-soft-gray font-medium px-1 mb-1 hidden md:block">아이 선택</p>
      {children.map((child) => {
        ...
        return (
          <button
            ...
            className={clsx(
              'flex flex-col md:flex-row items-center md:gap-2.5 p-2 md:p-2.5 rounded-2xl text-left transition-all w-full',
              isSelected
